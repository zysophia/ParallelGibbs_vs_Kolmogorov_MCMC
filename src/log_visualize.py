...
palette ={"Kolmogorov": "darkblue", "parallelGibbs": "darkred", "superGibbs": "darkgreen", \
        "parallelGibbs (T$\leftarrow 1$)": "darkorange", "superGibbs (T$\leftarrow 1$)": "yellowgreen"}

sns.set(font_scale = 5)

def lineplot_complexity_vs_eps_Voting():
    df = pd.read_csv("../data/compare_complexity_v1_241106.csv")
    bmax = 0.1
    chain = "logical"
    weights = 7
    ...
    plt.show()
    g.figure.savefig("../figures/complexity_vs_eps_Voting_2_v1_241106.png", dpi=200)
...
def lineplot_complexity_vs_eps_Voting_zoom():
    ...
    g = sns.lineplot(x="1/$\epsilon$", y="complexity", hue="algorithm", style = "algorithm", \
        errorbar=('ci', 0) ,data=df_sub, markers=True, markersize = 7, dashes=False, linewidth=1.5, palette=palette) 
    # g.set(ylim=(4, 11))
    plt.legend(loc='lower right')
    ax2 = plt.axes([0.14, 0.45, .4, .49], facecolor='gainsboro')
    sns.lineplot(x="1/$\epsilon$", y="complexity", hue="algorithm", style = "algorithm", \
        errorbar=('ci', 0) ,data=df_sub, markers=True, markersize = 7, dashes=False, linewidth=1.5, palette=palette, ax=ax2, legend=None) 
    ax2.set_yscale('log')
    ax2.set_xscale('log')
    ax2.set(xticklabels=[])  
    ax2.set(xlabel=None)
    ...
    plt.show()
    g.figure.savefig("../figures/complexity_vs_eps_Voting_zoom_2_v1_241106.png", dpi=200)
...
def scatterplot_complexity_vs_Z_Voting():
    ...
    df_sub = df_sub[(df_sub["algorithm"]=="Kolmogorov") | (df_sub["algorithm"]=="parallelGibbs")| (df_sub["algorithm"]=="superGibbs")]
    df_sub = df_sub[(df_sub["true_Q"]<1.6) & (df_sub["true_Q"]>1.2)]
    nn = 3
    ...
    sns.set_theme(style="darkgrid")
    g = sns.scatterplot(x="1/Z", y="complexity",  hue = "algorithm", style = "algorithm", data=df_sub, palette=palette) # style = "different_weights", 
    g.set(xlim=(0.152, 0.173))
    g.set(ylim=(10**4, 10**8))
    g.set_yscale('log')
    g.set_xscale('log')
    g.xaxis.set_minor_locator(mticker.LogLocator(base=10, subs=np.arange(0.005,1,0.005), numticks=5))
    ...
    plt.show()
    g.figure.savefig("../figures/complexity_vs_Z_2_v1_241106.png", dpi=200)


def lineplot_complexity_vs_eps_Ising(n=6):
    df = pd.read_csv("../data/isingcompare_complexity_v1_241106.csv")
    chain = "Ising"
    ...
    plt.show()
    g.figure.savefig(f"../figures/complexity_vs_eps_Ising{n}_2_v1_241106.png", dpi=200)
...
def lineplot_error_vs_eps_Voting():
    ...
    df_sub = df[(df["chain"]==chain)&(df["n"]==n)&(df["true_Q"]>1.3797)&(df["true_Q"]<1.3798)]
    ...
    g = sns.lineplot(x="$\epsilon$", y="absolute relative error", hue="algorithm", style = "algorithm", \
        errorbar=('ci', 35) ,data=df_sub, markers=True, dashes=False, palette=palette) # style = "different_weights", 
    # g.set(ylim=(5, 12))
    ...
    plt.show()
    g.figure.savefig("../figures/error_vs_epsilon_2_v1_241106.png", dpi=200)
