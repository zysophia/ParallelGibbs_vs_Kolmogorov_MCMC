...
if __name__ == "__main__":
    ...
    datalist = []
    f = open("../logs/output_v1_tmp_241106.log", 'r')
    ...
    print(df)  
    df.to_csv("../data/compare_complexity_v1_241106.csv", index=False)


    datalist = []
    f = open("../logs/isingoutput_v1_241106.log", 'r')
    for line in f.readlines():
        ss = line.split(",")
    ...
    df = pd.DataFrame(datalist, columns=["chain", "n", "beta_max", "epsilon", \
        "algorithm", "sample_complexity", "TPAsteps", "true_Q", "estimate_Q", "error"])
    print(df)  
    df.to_csv("../data/isingcompare_complexity_v1_241106.csv", index=False)
